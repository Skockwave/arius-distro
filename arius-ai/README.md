...
"내 컴퓨터에서 도는 자비스"라는 목표에는 로컬 모델이 잘 맞습니다. ARIUS는 `arius/llm/base.py` 의 백엔드 인터페이스로 설계되어 있어서, Ollama 백엔드(로컬 HTTP `localhost:11434`)를 붙이면 설정 한 줄로 전환할 수 있습니다 — 다음 단계로 추가 가능합니다.
...
│   ├── embeddings.py        # 임베딩(해싱 / sentence-transformers) + 문단 분할
